        ...
        <div>
            ...
            <div className="lg:hidden grid grid-rows-6 h-screen w-full mt-5">
                ...
                <div className="row-start-3 row-span-4 grid grid-rows-6 h-screen pt-2">
                    <div id='lu-btn' className="relative grid grid-rows-6 pt-3 h-full">
                        <div className="relative w-full row-start-1 row-span-2 flex justify-center items-center mt-3 h-full">
                            <div className="relative bg-blue-600 rounded-3xl w-8/12 xsm:w-6/12 grid grid-cols-2 overflow-hidden h-8 mb-2">
                                <button id='live-btn' onClick={toogleBtn} style={{backgroundColor: btnColors[0]}} className="flex justify-center flex-shrink-0 items-center object-fill font-semibold border-r-2 border-black w-full h-full bg-white text-sm xsm:text-base">Live</button>
                                <button id='upcoming-btn' onClick={()=>{setIsLive(!isLive);setButtonColors([btnColors[1], 'light-blue'])}} style={{backgroundColor: btnColors[1]}} className="flex justify-center items-center object-fill font-semibold  border-black w-full h-full bg-blue-500 text-sm xsm:text-base">Upcoming</button>
                            </div>
                        </div>
                        <div className="w-full row-start-3 row-span-4 px-2 py-3">
                            <div className="relative p-2 w-full h-full">
                                {isLive ? <div className="relative h-full"><UpcomingMatches/></div> : <div className="h-full"><LiveMatchList/></div>}
                            </div>
                        </div>
                    </div>
                </div>
